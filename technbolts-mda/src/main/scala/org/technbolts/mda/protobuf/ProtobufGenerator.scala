...
class ProtobufGenerator extends Generator {
  ...
  def generateField(builder:StringBuilder, field:ProtobufFieldModel):Unit = {
    builder.append(INDENT).append(field.mode).append(" ").append(field.fieldType).append(" ").append(field.name).append(" = ").append(field.ordinal)
  }
